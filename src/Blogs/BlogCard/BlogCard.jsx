    ...
    <div className="card w-96 bg-base-100 shadow-xl">
      ...
      <div className="card-body">
        ...
        <div className="card-actions mt-4">
          <button onClick={()=>handleAddToWishlist(blog)} className="rounded-md bg-[#d1d1d1] hover:bg-[#8dcc78] px-6 py-2 font-semibold">Add to wishlist</button>
          <Link to={`/blog/${_id}`}><button className="rounded-md bg-[#d1d1d1] hover:bg-[#8dcc78] px-6 py-2 font-semibold">Details</button></Link>
        </div>
      </div>
    </div>
  );
